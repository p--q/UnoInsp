...
class ObjInsp:  # XSCRIPTCONTEXTを引数にしてインスタンス化する。第二引数をTrueにするとローカルのAPIリファレンスへのリンクになる。
    ...
    def itree(self, obj, t_supr=None):  # アンカータグをつけて出力。IPython Notebookでの使用を想定。t_suprはタプル。
        self._init(t_supr)  # 初期化関数
    ...
    def wtree(self, obj, t_supr=None):  # ウェブブラウザの新たなタブに出力。t_suprはタプル。
        self._init(t_supr)  # 初期化関数
        self._output_setting()  # IDL名にリンクをつけて出力するための設定。
...
#         self.st_omi = ST_OMI.copy()  # 結果を出力しないインターフェイス名の集合の初期化。
        ...
        if t_supr:  # 第2引数があるとき
            if isinstance(t_supr, tuple):  # t_suprがタプルのとき
                st_supr = set([i.replace(CSS, "") for i in t_supr])  # "com.sun.star"を省いて集合に変換。
                if "core" in st_supr:  # coreというキーワードがあるときはST_OMIの要素に置換する。
                    st_supr.remove("core")
                    st_supr.update(ST_OMI)
#                 self.st_omi = st_supr.symmetric_difference(ST_OMI)  # デフォルトでcoreインターフェースを出力しないとき。st_suprとST_OMIに共通しない要素を取得。
        ...
        if self.stack:  # 起点となるサービスかインターフェイスがあるとき。
            ...
            while self.stack:  # スタックがある間実行。
                ...
                if typcls == INTERFACE_METHOD:  # jがメソッドのとき。
                    typ = self._format_type(j.ReturnType.Name.replace(CSS, ""))  # 戻り値の型を取得。
                    ...
                    if not stack2:  # 引数がないとき。
                        ...
                        typ2 = self._format_type(k.Type.Name.replace(CSS, ""))  # 戻り値の型を取得。
                    ...
                    if typcls == INTERFACE:  # インターフェイスのとき。XInterfaceTypeDescription2インターフェイスをもつTypeDescriptionオブジェクト。
                        ...
                        typ = self._format_type(j.getPropertyTypeDescription().Name.replace(CSS, ""))  # 属性の型を取得。
                        ...
                        typ = self._format_type(j.Type.Name.replace(CSS, ""))  # 戻り値の型を取得。
